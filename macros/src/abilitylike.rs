...
pub(crate) fn abilitylike_inner(ast: &DeriveInput) -> TokenStream {
    ...
    let crate_path = if let Ok(found_crate) = crate_name("leafwing_abilities") {
        // The crate was found in the Cargo.toml
        match found_crate {
            FoundCrate::Itself => quote!(leafwing_abilities),
        ...
        quote!(leafwing_abilities)
    };

    quote! {
        impl #impl_generics #crate_path::Abilitylike for #enum_name #type_generics #where_clause {}
    }
}
